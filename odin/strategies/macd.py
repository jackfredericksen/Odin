...
class MACDStrategy(Strategy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"Initialized MACD strategy: fast={fast_period}, "
    ...
    def generate_signal(self, data: pd.DataFrame) -> Signal:
        ...
        signal_type, reasoning = self._analyze_macd_signals(
            current_row,
            previous_row,
            current_macd_above_signal,
            current_macd_above_zero,
        )
    ...
    def _analyze_macd_signals(
        ...
        if abs(histogram_slope) > 0.001:  # Significant histogram change
            if (
                ...
                and histogram_slope < 0
                and macd_momentum < 0
                and not current_macd_above_signal
            ):
                return (
                    SignalType.SELL,
    ...
    def _detect_divergence(
        self, row: pd.Series, price_momentum: float, macd_momentum: float
    ) -> bool:
        ...
        return (
            abs(price_momentum) > divergence_threshold
            and abs(macd_momentum) > divergence_threshold
            and (price_momentum * macd_momentum) < 0
        )  # Opposite signs
    ...
    def update_parameters(self, parameters: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"Updated parameters: fast={self.fast_period}, "
