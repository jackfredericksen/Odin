...
class AIAdaptiveStrategy(BaseStrategy):
    ...
    def generate_signal(self, data: pd.DataFrame) -> Optional[Signal]:
        ...
        try:
            ...
            if (
                self.data_point_count - self.last_regime_update
            ) >= self.regime_update_frequency:
                self._update_regime_detection(data)
                self.last_regime_update = self.data_point_count

            # Rebalance strategy weights periodically
            if (
                self.data_point_count - self.last_strategy_rebalance
            ) >= self.strategy_rebalance_frequency:
    ...
    def _update_regime_detection(self, data: pd.DataFrame):
        ...
        try:
            ...
            self.logger.info(
                f"Detected regime: {regime} (confidence: {confidence:.3f})"
            )
            ...
            if confidence >= self.min_regime_confidence:
                ...
                self.logger.warning(
                    f"Low regime confidence ({confidence:.3f}) - maintaining current strategies"
                )
    ...
    def _create_ai_enhanced_signal(
        self, combined_signal: Dict, data: pd.DataFrame
    ) -> Signal:
        """Create final signal with AI enhancements"""
        try:
            ...
            metadata = {
                ...
                "contributing_strategies": combined_signal.get(
                    "contributing_strategies", []
                ),
                "active_strategy_count": combined_signal.get(
    ...
    def _apply_ai_enhancements(
        self, strength: float, confidence: float, data: pd.DataFrame
    ) -> tuple:
    ...
    def _track_signal_performance(self, signal: Signal, data: pd.DataFrame):
        ...
        try:
            performance_entry = {
                ...
                "contributing_strategies": signal.metadata.get(
                    "contributing_strategies", []
                ),
            }
        ...
        try:
            ...
            analytics = {
                "regime_detection": {
                    ...
                    "regime_distribution": regime_stats.get(
                        "regime_distribution_30d", {}
                    ),
    ...
    def _calculate_recent_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_metrics = {
                ...
                "active_signal_ratio": (
                    (len(strengths) / len(recent_signals)) if recent_signals else 0
                ),
            }
    ...
    def _assess_model_health(self) -> Dict[str, Any]:
        ...
        try:
            health_metrics = {
                ...
                "strategy_manager_active": len(self.strategy_manager.active_strategies)
                > 0,
    ...
    def _calculate_regime_accuracy(self) -> float:
        ...
        try:
            ...
            avg_confidence = sum(
                entry["confidence"] for entry in recent_accuracy
            ) / len(recent_accuracy)
    ...
    def get_strategy_recommendations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            regime_performance = (
                self.strategy_manager.get_strategy_performance_by_regime(
    ...
    def _get_risk_recommendations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            regime_mapping = self.strategy_manager.regime_strategy_map.get(
                current_regime
            )
            ...
            risk_recommendations = {
                ...
                "position_size_limit": regime_mapping.max_exposure
                * 0.8,  # Conservative
                "stop_loss_recommendation": self._get_stop_loss_recommendation(
                    current_regime
                ),
                "diversification_advice": self._get_diversification_advice(
                    current_regime
                ),
            }
    ...
    def _get_position_sizing_recommendations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sizing_recommendations = {
                ...
                "size_rationale": self._get_sizing_rationale(
