...
class SwingTradingStrategy(Strategy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"Initialized Swing Trading strategy for {primary_timeframe} timeframe"
        )
        self.logger.info(
            f"RSI levels: {rsi_oversold}/{rsi_overbought}, MA: {ma_fast}/{ma_slow}"
        )
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df["sma_200"] = (
            df["close"].rolling(window=200, min_periods=1).mean()
        )  # Long-term trend
        ...
        df["bb_middle"] = (
            df["close"].rolling(window=self.bb_period, min_periods=1).mean()
        )
        ...
        df["market_volatility"] = (
            df["close"].rolling(window=20, min_periods=1).std() / df["close"]
        )
    ...
    def _calculate_support_resistance(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(df)):
            ...
            if supports_below:
                ...
                df.iloc[i, df.columns.get_loc("support_distance")] = (
            ...
            if resistances_above:
                ...
                df.iloc[i, df.columns.get_loc("resistance_distance")] = (
    ...
    def generate_signal(self, data: pd.DataFrame) -> Signal:
        ...
        indicators = {
            ...
            "rsi_smooth": float(
                current_row.get("rsi_smooth", current_row.get("rsi", 50))
            ),
        ...
        self.logger.debug(
            f"Swing signal: {signal_type.value} "
    ...
    def _check_exit_conditions(
        self, current_row: pd.Series, data: pd.DataFrame
    ) -> Optional[Signal]:
        ...
        if (
            self.entry_time
            and (current_time - self.entry_time).days >= self.max_hold_days
        ):
            return Signal(
                signal=(
                    SignalType.SELL
        ...
        if self.stop_loss:
            if (
                self.current_position == "long" and current_price <= self.stop_loss
            ) or (self.current_position == "short" and current_price >= self.stop_loss):
        ...
        if self.take_profit:
            if (
                self.current_position == "long" and current_price >= self.take_profit
            ) or (
    ...
    def _analyze_long_setup(
        ...
        resistance_distance = self._get_nearest_resistance_distance(
            current_row["close"]
        )
    ...
    def _prepare_long_position(self, current_row: pd.Series, setup: Dict[str, Any]):
        ...
        self.logger.info(
            f"Prepared long position at {current_price:.2f}, "
    ...
    def _prepare_short_position(self, current_row: pd.Series, setup: Dict[str, Any]):
        ...
        self.logger.info(
            f"Prepared short position at {current_price:.2f}, "
            f"SL: {self.stop_loss:.2f}, TP: {self.take_profit:.2f}, "
            f"R/R: {self.current_risk_reward:.2f}"
        )
    ...
    def _close_position(self):
        ...
        if self.current_position:
            self.logger.info(
                f"Closing {self.current_position} position at {self.entry_price}"
            )
    ...
    def _calculate_swing_confidence(
        self, row: pd.Series, signal_type: SignalType, data: pd.DataFrame
    ) -> float:
        ...
        if signal_type == SignalType.BUY:
            rsi_confidence = (
                max(0, (self.rsi_oversold - rsi) / self.rsi_oversold)
                if self.rsi_oversold > 0
                else 0
            )
        else:
            rsi_confidence = (
                max(0, (rsi - self.rsi_overbought) / (100 - self.rsi_overbought))
        ...
        # Volume confirmation
        volume_confidence = (
            min((volume_ratio - 1) * 0.5, 0.5) if volume_ratio > 1 else 0
        )
    ...
    def _calculate_swing_risk(
        ...
        elif self.market_structure == "ranging" and signal_type in [
            SignalType.BUY,
            SignalType.SELL,
        ]:
        ...
        return max(
            0.0,
            min(1.0, sum(risk_factors) / len(risk_factors) if risk_factors else 0.0),
        )
    ...
    def get_current_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                ...
                "last_signal": {
                    "type": (
                        self.last_signal.signal.value if self.last_signal else "none"
                    ),
                    "confidence": (
                        self.last_signal.confidence if self.last_signal else 0
                    ),
                    "timestamp": (
    ...
    def validate_parameters(self) -> bool:
        ...
        try:
            ...
            assert (
                1.0 <= self.volume_threshold <= 5.0
            ), "Volume threshold must be between 1.0 and 5.0"
            assert (
                1.0 <= self.min_risk_reward <= 10.0
            ), "Min risk/reward must be between 1.0 and 10.0"
            assert (
                1 <= self.max_hold_days <= 30
            ), "Max hold days must be between 1 and 30"
            assert (
                0.005 <= self.stop_loss_pct <= 0.20
            ), "Stop loss must be between 0.5% and 20%"
            assert (
                0.01 <= self.take_profit_pct <= 0.50
            ), "Take profit must be between 1% and 50%"
    ...
    def get_optimization_suggestions(
        ...
        if performance_data:
            ...
            if win_rate < 0.4:
                suggestions.append(
                    "Consider tightening entry criteria - low win rate detected"
                )
                suggestions.append(
                    "Try adjusting RSI levels or requiring more confirmations"
                )

            if win_rate > 0.8:
                suggestions.append(
                    "Win rate very high - consider loosening criteria for more opportunities"
                )

            if avg_trade_duration > self.max_hold_days * 20:  # hours
                suggestions.append(
                    "Trades holding too long - consider tighter profit targets"
                )
