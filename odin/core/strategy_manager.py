...
from .exceptions import StrategyConfigurationException, StrategyException
from .models import PriceData, StrategySignal
...
class EnhancedStrategyManager:
    ...
    def __init__(self, database: Database):
        ...
        self.ai_selector = AIStrategySelector(
            regime_detector=self.regime_detector, strategy_scorer=self.strategy_scorer
        )
    ...
    async def _perform_initial_strategy_selection(self):
        ...
        try:
            ...
            if self.active_strategy_id:
                self.strategy_pool[self.active_strategy_id]["last_used"] = datetime.now(
                    timezone.utc
                )

            logger.info(
                f"Initial strategy selected: {self.active_strategy_id} "
    ...
    async def _evaluate_and_potentially_switch_strategy(self):
        ...
        try:
            ...
            if should_switch:
                ...
                logger.info(
                    f"Strategy switched to: {recommended_strategy} "
    ...
    def _should_switch_strategy(
        self, recommended_strategy: str, recommended_confidence: float
    ) -> bool:
        ...
        confidence_improvement = (
            recommended_confidence - self.active_strategy_confidence
        )
    ...
    async def generate_trading_signal(
        ...
        try:
            ...
            if signal:
                    ...
                    signal.indicators.update(
                        {
                            ...
                            "regime_confidence": self.market_regime.get(
                                "confidence", 0.0
                            ),
                        }
                    )
                ...
                logger.info(
                    f"Signal generated by {self.active_strategy_id}: {signal.signal.value}"
                )
        ...
        try:
                ...
                strategy_status.append(
                    {
                        ...
                        "performance": {
                            "return_24h": round(
                                performance.get("total_return", 0.0), 2
                            ),
            ...
            return {
                "active_strategy": {
                    ...
                    "name": (
                        self.strategy_pool[self.active_strategy_id]["name"]
                        if self.active_strategy_id
                        else "None"
                    ),
                    "confidence": round(self.active_strategy_confidence, 3),
                    "type": (
                        self.strategy_pool[self.active_strategy_id]["type"]
                        if self.active_strategy_id
                        else "None"
                    ),
                },
                ...
                "system_stats": {
                    ...
                    "next_evaluation": self.last_evaluation
                    + timedelta(seconds=self.config["evaluation_frequency"]),
    ...
    async def get_ai_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if regime_confidence > 0.7:
                insights.append(
                    f"High confidence {regime} market detected - strategy selection optimized"
                )
            elif regime_confidence > 0.5:
                insights.append(
                    f"Moderate confidence {regime} market - monitoring for changes"
                )
            else:
                insights.append(
                    "Uncertain market conditions - using conservative strategy selection"
                )
            ...
            if self.strategy_scores:
                ...
                if best_strategy[1] > 0.8:
                    insights.append(
                        f"Strong strategy signals detected - {best_strategy[0]} highly favored"
                    )

                score_spread = best_strategy[1] - worst_strategy[1]
                if score_spread < 0.2:
                    insights.append(
                        "Strategy scores are close - market conditions may be transitioning"
                    )
            ...
            if active_strategy_id and active_strategy_id in self.strategy_performance:
                ...
                if perf.get("total_return", 0) > 0:
                    ...
                    insights.append(
                        "Current strategy underperforming - evaluation increased"
                    )
        ...
        try:
            ...
            if recent_switches > 3:
                recommendations.append(
                    "Consider increasing switch threshold - frequent strategy changes detected"
                )
            ...
            if avg_performance < -1:
                recommendations.append(
                    "Overall strategy performance declining - consider parameter optimization"
                )
            elif avg_performance > 2:
                recommendations.append(
                    "Strong performance across strategies - current market conditions favorable"
                )
            ...
            if regime_conf < 0.5:
                recommendations.append(
                    "Low regime confidence - consider more conservative position sizing"
                )
    ...
    async def _calculate_strategy_performance(self, strategy_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            trade_returns = [
                trade.get("pnl", 0) / 1000 for trade in trades
            ]  # Normalize

            total_return = (
                (total_pnl / (total_trades * 1000) * 100) if total_trades > 0 else 0
            )
