...
from .exceptions import ErrorCode, ErrorSeverity, OdinException
    ...
    async def _create_tables(self):
        ...
        for i, table_sql in enumerate(tables):
            ...
            if not result.success:
                ...
                logger.error(
                    f"Error detail: '{result.error}' (type: {type(result.error)})"
                )
                raise OdinException(
...
class PriceRepository:
    ...
    async def get_price_history(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        query = (
            "SELECT * FROM bitcoin_prices WHERE timestamp >= ? ORDER BY timestamp ASC"
        )
...
class StrategyRepository:
    ...
    async def update_signal_execution(self, signal_id: str, execution_id: str) -> bool:
        ...
        query = (
            "UPDATE strategy_signals SET executed = 1, execution_id = ? WHERE id = ?"
        )
...
class RepositoryManager:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        result = await self.db.execute_query(
            "SELECT COUNT(*) as count FROM bitcoin_prices", fetch_one=True
        )
        stats["price_records"] = (
            result.data["count"] if result.success and result.data else 0
        )

        # Trade count
        result = await self.db.execute_query(
            "SELECT COUNT(*) as count FROM trades", fetch_one=True
        )
        stats["trade_records"] = (
            result.data["count"] if result.success and result.data else 0
        )
        ...
        # Signal count
        result = await self.db.execute_query(
            "SELECT COUNT(*) as count FROM strategy_signals", fetch_one=True
        )
        stats["signal_records"] = (
            result.data["count"] if result.success and result.data else 0
        )
