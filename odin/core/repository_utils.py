...
class DataExporter:
    ...
    async def export_price_data(
        ...
        try:
            ...
            result = await price_repo.find_by_date_range(
    ...
    async def export_trade_history(
        self, strategy_id: Optional[str] = None, days: int = 30, format: str = "json"
    ) -> Dict[str, Any]:
...
class PerformanceAnalyzer:
    ...
    def _calculate_risk_metrics(self, trades: List[Any]) -> Dict[str, Any]:
        ...
        if negative_pnls and len(negative_pnls) > 1:
            ...
            downside_variance = sum(
                (pnl - mean_negative) ** 2 for pnl in negative_pnls
            ) / (len(negative_pnls) - 1)
            downside_deviation = downside_variance**0.5
            sortino_ratio = (
                (sum(pnls) / len(pnls)) / downside_deviation
    ...
    async def compare_strategies(
        self, strategy_ids: List[str], days: int = 30
    ) -> Dict[str, Any]:
...
class DataMigrator:
    ...
    async def add_missing_indexes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for index_sql in additional_indexes:
                result = await self.repo_manager.db_manager.execute_query(
                    index_sql, fetch_all=False
                )
                results.append(
                    {
                        "index": (
    ...
    async def cleanup_orphaned_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cleanup_operations.append(
                {
                    ...
                    "records_removed": (
                        orphaned_signals.rows_affected
                        if orphaned_signals.success
                        else 0
                    ),
            ...
            cleanup_operations.append(
                {
                    ...
                    "records_removed": (
                        orphaned_positions.rows_affected
                        if orphaned_positions.success
                        else 0
                    ),
            ...
            total_cleaned = sum(
                op["records_removed"] for op in cleanup_operations if op["success"]
            )
    ...
    async def _check_data_freshness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if latest_result.success and latest_result.data:
                ...
                if latest_time:
                    age = datetime.now(timezone.utc) - latest_time.replace(
                        tzinfo=timezone.utc
                    )
                    return {
            ...
            return {"recent_data": False, "error": str(e)}
        ...
        try:
            ...
            test_queries = [
                ...
                (
                    ...
                    "SELECT strategy_id, COUNT(*) FROM strategy_signals GROUP BY strategy_id",
                ),
            ]
            ...
            return {"avg_query_time": None, "error": str(e)}
    ...
    async def _check_data_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if orphaned_trades.success:
                orphaned_count = (
                    orphaned_trades.data["count"] if orphaned_trades.data else 0
                )
            ...
            if negative_prices.success:
                negative_count = (
                    negative_prices.data["count"] if negative_prices.data else 0
                )
                integrity_checks.append(
                    {
            ...
            if future_timestamps.success:
                future_count = (
                    future_timestamps.data["count"] if future_timestamps.data else 0
                )
                integrity_checks.append(
                    {
            ...
            failed_checks = [
                check["details"] for check in integrity_checks if not check["passed"]
            ]
            ...
            if os.path.exists(db_path):
                ...
                return {
                    ...
                    "usage_percentage": min(
                        (file_size / (100 * 1024 * 1024)) * 100, 100
                    ),  # Assume 100MB is 100%
                    "table_info": table_info.data if table_info.success else [],
                    "storage_path": db_path,
                }
            else:
                return {"error": "Database file not found", "storage_path": db_path}
...
async def export_price_data_to_json(
...
async def analyze_all_strategies(days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        strategy_ids = (
            [row["strategy_id"] for row in all_performance.data]
