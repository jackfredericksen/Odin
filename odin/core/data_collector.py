        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    f"{self.base_url}/products/BTC-USD/ticker"
                ) as response:
    ...
    async def get_depth(self) -> Optional[MarketDepth]:
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    f"{self.base_url}/products/BTC-USD/book?level=2"
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        raise DataSourceException(
                            self.name, f"Depth API error: {response.status}"
                        )
...
class TechnicalIndicators:
    ...
    def macd(
        ...
        macd_line = [
            fast_val - slow_val
            for fast_val, slow_val in zip(ema_fast_aligned, ema_slow)
        ]
...
class DataCollector:
    ...
    async def _collect_data(self):
        ...
        for source in self.data_sources:
            ...
            try:
                ...
                if price_data:
                    logger.debug(
                        f"Got price data from {source.name}: ${price_data.price}"
                    )
                    break
            except Exception as e:
        ...
        try:
            self.database.add_price_data(
                price_data.timestamp, price_data.price, price_data.volume
            )
        except Exception as e:
    ...
    async def _validate_price_data(self, price_data: PriceData):
        ...
        if price_data.price < 1000 or price_data.price > 1000000:
            raise DataValidationException(
                f"Price out of reasonable range: ${price_data.price}"
            )
        ...
        if self.latest_price:
            time_diff = (
                price_data.timestamp - self.latest_price.timestamp
            ).total_seconds()
            if time_diff > 0:  # Only check if newer
                price_change = (
                    abs(price_data.price - self.latest_price.price)
    ...
    def get_source_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for source in self.data_sources:
            status[source.name] = {
                ...
                "last_update": (
                    source.last_update.isoformat() if source.last_update else None
                ),
            }
        return status
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_collection": (
                self.latest_price.timestamp.isoformat() if self.latest_price else None
            ),
