...
class EnhancedBitcoinDataCollector:
    ...
    def _init_database(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_timestamp ON bitcoin_enhanced(timestamp)"
        )
        conn.commit()
        conn.close()

    def get_yfinance_data(
        self, start_date: str = "2015-01-01", end_date: str = None
    ) -> pd.DataFrame:
        """
    ...
    def get_coingecko_data(self, days: int = 365) -> pd.DataFrame:
        ...
        try:
            ...
            df_data = []
            for i, (price_data, mc_data, vol_data) in enumerate(
                zip(prices, market_caps, volumes)
            ):
            ...
            if current_response.status_code == 200:
                ...
                if not df.empty:
                    ...
                    df.loc[latest_idx, "price_change_24h"] = market_data.get(
                        "price_change_24h", {}
                    ).get("usd", 0)
                    df.loc[latest_idx, "price_change_percentage_24h"] = market_data.get(
                        "price_change_percentage_24h", {}
                    ).get("usd", 0)
                    df.loc[latest_idx, "market_cap_rank"] = market_data.get(
                        "market_cap_rank", 1
                    )
    ...
    def get_historical_complete_dataset(self) -> pd.DataFrame:
        ...
        try:
            ...
            early_cg_data = self.get_coingecko_data(
                days="max"
            )  # CoinGecko has data from 2013
    ...
    def _merge_data_sources(
        ...
        cg_current: pd.DataFrame,
        cg_historical: pd.DataFrame,
    ) -> pd.DataFrame:
        ...
        for field in cg_fields:
            if field in cg_current.columns:
                base_df[field] = base_df.index.map(
                    lambda x: (
                        cg_current.loc[x, field] if x in cg_current.index else np.nan
                    )
                )
...
def setup_enhanced_data_collection():
    ...
    if not complete_data.empty:
        ...
        print(
            f"ML-ready dataset: {len(ml_features)} samples, {len(ml_features.columns)} features"
        )
