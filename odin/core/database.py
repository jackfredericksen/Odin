...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_prices_timestamp ON bitcoin_prices(timestamp DESC)"
                )
                cursor.execute(
    ...
    def get_recent_trades(
        self, limit: int = 10, strategy_id: str = None
    ) -> List[Dict[str, Any]]:
        """Get recent trades."""
        try:
            with sqlite3.connect(self.db_path) as conn:
        ...
        **kwargs,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                allocation_json = (
                    json.dumps(kwargs.get("allocation"))
                    if kwargs.get("allocation")
                    else None
                )
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Get data ranges
                cursor.execute(
                    "SELECT MIN(timestamp), MAX(timestamp) FROM bitcoin_prices"
                )
