...
class PortfolioManager:
    ...
    async def execute_trade(self, trade: TradeExecution, current_price: float):
        ...
        try:
            ...
            if trade.side == OrderSide.BUY:
                ...
                logger.info(
                    f"BUY executed: {trade.quantity:.6f} BTC at ${trade.price:.2f}"
                )
                ...
                logger.info(
                    f"SELL executed: {trade.quantity:.6f} BTC at ${trade.price:.2f}"
                )
    ...
    async def get_portfolio_summary(self, current_price: float) -> PortfolioSummary:
        ...
        try:
            ...
            daily_pnl_percent = (
                daily_pnl / self.initial_capital if self.initial_capital > 0 else 0
            )
    ...
    async def get_allocation(self, current_price: float) -> Allocation:
        ...
        try:
            ...
            strategy_allocations = await self._calculate_strategy_allocations(
                current_price
            )
            ...
            return Allocation(
                ...
                cash_percentage=(
                    self.cash_balance / total_value if total_value > 0 else 1
                ),
            )
    ...
    async def get_performance_metrics(self, hours: int = 24) -> PerformanceMetrics:
        ...
        try:
            ...
            annualized_return = (
                ((1 + total_return) ** (365 / days_elapsed) - 1)
                if days_elapsed > 0
                else 0
            )
            volatility = (
                stdev(daily_returns) * math.sqrt(365) if len(daily_returns) > 1 else 0
            )
            ...
            downside_deviation = (
                stdev(downside_returns) * math.sqrt(365)
            ...
            avg_trade_return = (
                mean([t.total_cost / (t.quantity * t.price) - 1 for t in trades])
                if trades
                else 0
            )
    ...
    async def get_performance_attribution(
        ...
        try:
            ...
            for strategy_name in self.target_allocations.keys():
                ...
                strategy_trades = await self.database.get_strategy_trades(
                    strategy_name, hours
                )
                allocation = await self._get_strategy_allocation_percentage(
                    strategy_name
                )
                ...
                return_volatility = (
                    stdev(strategy_returns) if len(strategy_returns) > 1 else 0
                )
                sharpe_ratio = (
                    avg_return / return_volatility if return_volatility > 0 else 0
                )
                ...
                return_contribution = (
                    (total_pnl / portfolio_value) * 100 if portfolio_value > 0 else 0
                )
                ...
                win_rate = (
                    winning_trades / len(strategy_returns) if strategy_returns else 0
                )
    ...
    async def get_rebalance_recommendations(
        ...
        try:
            recommendations = []
            current_allocations = await self._calculate_strategy_allocations(
                current_price
            )
            total_value = await self.get_total_value(current_price)
    ...
    async def execute_rebalance(self, current_price: float) -> Dict[str, Any]:
        ...
        try:
            ...
            raise PortfolioManagerException(
                message=f"Failed to execute rebalancing: {str(e)}", original_exception=e
            )
    ...
    async def _calculate_strategy_allocations(
        ...
        for strategy_name in self.target_allocations.keys():
            ...
            strategy_value = (
                total_value * self.target_allocations[strategy_name]
            )  # Placeholder
