    ...
    def calculate_pnl(self):
        ...
        if self.entry_price and self.exit_price:
            ...
            if self.entry_price > 0:
                self.pnl_percentage = (
                    self.pnl / (self.entry_price * self.amount)
                ) * 100
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "market_regime_at_entry": (
                self.market_regime_at_entry.value
                if self.market_regime_at_entry
                else None
            ),
            "expected_duration": self.expected_duration,
...
class AISystemStatus:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_model_update": (
                self.last_model_update.isoformat() if self.last_model_update else None
            ),
...
def create_price_data_from_ohlcv(
    timestamp: datetime,
    open_price: float,
    high: float,
    low: float,
...
def create_signal_from_strategy(
...
def validate_signal(signal: StrategySignal) -> bool:
    ...
    if (
        not isinstance(signal.risk_score, (int, float))
        or not 0 <= signal.risk_score <= 1
    ):
        return False
