...
from .models import (
    RiskLimits, RiskMetrics, DrawdownAlert, Position, TradeOrder, 
    PortfolioSummary, OrderSide, PositionType
)
from .exceptions import (
    RiskManagerException, RiskLimitExceededException, 
    DrawdownLimitException, PositionSizeException
)
...
class RiskMonitor:
    ...
    async def _check_exposure(self, risk_metrics: RiskMetrics):
        ...
        if exposure >= self.alert_thresholds["exposure_critical"]:
            await self._trigger_alert("exposure_critical", {
            ...
            await self._trigger_alert("exposure_warning", {
                "current_exposure": exposure,
    ...
    async def _check_var_breaches(self, risk_metrics: RiskMetrics):
        ...
        if risk_metrics.var_1d_95:
            ...
            if actual_pnl < 0 and abs(actual_pnl) > abs(risk_metrics.var_1d_95):
                await self._trigger_alert("var_breach", {
                    "actual_pnl": actual_pnl,
                    "var_95": risk_metrics.var_1d_95,
                    "breach_amount": abs(actual_pnl) - abs(risk_metrics.var_1d_95),
                    "message": "VaR limit breached",
                    "recommended_actions": ["Review risk models", "Assess portfolio"]
                })
    
    async def _check_concentrations(self, risk_metrics: RiskMetrics):
        """Check concentration risks."""
        max_concentration = max(risk_metrics.strategy_concentrations.values()) if risk_metrics.strategy_concentrations else 0
        
        if max_concentration > 0.7:  # 70% in single strategy
            strategy_name = max(risk_metrics.strategy_concentrations.items(), key=lambda x: x[1])[0]
            await self._trigger_alert("concentration_risk", {
                "strategy_name": strategy_name,
                "concentration": max_concentration,
                "message": f"High concentration in {strategy_name}",
                "recommended_actions": ["Rebalance portfolio", "Reduce strategy allocation"]
            })
...
class RiskManager:
    ...
    async def validate_trade(self, order: TradeOrder, current_price: float) -> RiskCalculation:
        """
        ...
        try:
            ...
            current_exposure = await self._calculate_current_exposure(current_positions, current_price)
            ...
            if position_size_pct > self.risk_limits.max_trade_size:
                return RiskCalculation(
                    is_allowed=False,
                    risk_score=1.0,
                    max_position_size=self.risk_limits.max_trade_size * portfolio_value / current_price,
                    stop_loss_price=None,
                    take_profit_price=None,
            ...
            if new_exposure > self.risk_limits.max_position_size:
                return RiskCalculation(
                    is_allowed=False,
                    risk_score=1.0,
                    max_position_size=(self.risk_limits.max_position_size - current_exposure) * portfolio_value / current_price,
                    stop_loss_price=None,
                    take_profit_price=None,
            ...
            strategy_allocation = await self._get_strategy_allocation(order.strategy_name)
            strategy_position_value = 0
            if order.side == OrderSide.BUY:
                strategy_position_value = position_value
            
            new_strategy_allocation = (strategy_allocation + strategy_position_value) / portfolio_value
            if new_strategy_allocation > self.risk_limits.max_strategy_allocation:
                max_additional = (self.risk_limits.max_strategy_allocation * portfolio_value) - strategy_allocation
                return RiskCalculation(
                    is_allowed=False,
                    risk_score=0.9,
    ...
    def _calculate_risk_score(
        ...
        daily_loss_risk = min(1.0, abs(min(0, daily_pnl_pct)) / self.risk_limits.max_daily_loss)
    ...
    def _calculate_stop_loss(self, order: TradeOrder, current_price: float) -> Optional[float]:
    ...
    def _calculate_take_profit(self, order: TradeOrder, current_price: float) -> Optional[float]:
    ...
    async def calculate_var(
        ...
        try:
            ...
            if method == "historical":
                return await self._calculate_historical_var(returns, confidence_level, holding_period)
            elif method == "parametric":
                return await self._calculate_parametric_var(returns, confidence_level, holding_period)
            else:
    ...
    async def _calculate_historical_var(
        ...
        expected_shortfall = mean(tail_losses) * scaling_factor if tail_losses else var_95
    ...
    async def _calculate_current_exposure(self, positions: List[Position], current_price: float) -> float:
    ...
    async def _calculate_market_correlation(self) -> Tuple[Optional[float], Optional[float]]:
