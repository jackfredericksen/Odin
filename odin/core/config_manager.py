...
class OdinConfig:
    ...
    def validate(self) -> bool:
        ...
        if total_allocation > 1.0:
            errors.append(
                f"Total strategy allocation ({total_allocation:.2f}) exceeds 1.0"
            )
...
class ConfigManager:
    ...
    def load_config(self) -> OdinConfig:
        ...
        if self._config is None:
            ...
            logger.info(
                f"Configuration loaded successfully ({self._config.environment})"
            )
    ...
    def is_live_trading_enabled(self) -> bool:
        ...
        return (
            self._config.trading.enable_live_trading
            and not self._config.trading.paper_trading
            and self._config.validate()
        )
