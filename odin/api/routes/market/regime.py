...
async def get_market_regime(
    ...
    try:
        ...
        # Volatility analysis
        returns = [(market_data[i].price / market_data[i+1].price - 1) for i in range(min(30, len(market_data)-1))]
        ...
        current_recommendation = strategy_recommendations.get(regime, strategy_recommendations["sideways"])
...
async def get_market_conditions(
    ...
    try:
        ...
        price_metrics = {
            ...
            "24h_range_percent": ((current_data.high_24h - current_data.low_24h) / current_data.low_24h * 100) if current_data.low_24h else 0
        }
        ...
        volume_metrics = {
            ...
            "volume_ratio": current_data.volume_24h / avg_volume if avg_volume > 0 else 1,
            "volume_trend": "high" if current_data.volume_24h > avg_volume * 1.5 else "low" if current_data.volume_24h < avg_volume * 0.5 else "normal"
        }
        
        # Volatility analysis
        if len(prices) >= 30:
            returns = [(prices[i] / prices[i+1] - 1) for i in range(29)]
            realized_vol = (sum([r**2 for r in returns]) / len(returns)) ** 0.5 * (365**0.5)
        ...
        return {
            ...
            "data_quality": {
                ...
                "last_update": current_data.timestamp.isoformat() if hasattr(current_data, 'timestamp') else datetime.utcnow().isoformat(),
                "source": current_data.source if hasattr(current_data, 'source') else "unknown"
            },
            "timestamp": datetime.utcnow().isoformat(),
            "status": "success"
        }
...
async def get_volatility_analysis(
    ...
    try:
        ...
        if hasattr(price_data[0], 'high') and hasattr(price_data[0], 'low'):
            hl_ratios = [point.high / point.low for point in price_data if point.high and point.low]
            if hl_ratios:
                parkinson_vol = (sum([math.log(ratio)**2 for ratio in hl_ratios[:30]]) / len(hl_ratios[:30])) ** 0.5 * (365**0.5)
        ...
        volatility_analysis = {
            ...
            "volatility_trend": {
                "direction": "increasing" if len(vol_7d) > 1 and vol_7d[-1] > vol_7d[-2] else "decreasing",
                "rate_of_change": (vol_7d[-1] - vol_7d[-2]) / vol_7d[-2] if len(vol_7d) > 1 and vol_7d[-2] != 0 else 0
            },
            "volatility_regime": {
            ...
            "historical_comparison": {
                "vs_1_month_avg": (current_vol / (sum(vol_30d) / len(vol_30d)) - 1) if vol_30d else 0,
...
def calculate_volatility_percentile(volatility: float) -> float:
    ...
    vol_percentiles = {
        0.2: 10, 0.3: 25, 0.4: 40, 0.5: 50, 0.6: 60, 
        0.8: 75, 1.0: 85, 1.5: 95, 2.0: 99
    }
...
def assess_market_sentiment(change_24h: float, volume_ratio: float, vol_percentile: float, momentum: float) -> Dict[str, Any]:
...
def calculate_rolling_volatility(returns: list, window: int) -> list:
    ...
    for i in range(window, len(returns)):
        ...
        vol = (sum([r**2 for r in window_returns]) / len(window_returns)) ** 0.5 * (365**0.5)
