...
async def get_trading_fees(
    period: str = Query(
        "30d", description="Period for fee analysis (7d, 30d, 90d, 1y)"
    ),
    ...
    try:
        ...
        fee_analysis = await portfolio_manager.get_fee_analysis(
            user_id=current_user["username"], hours=hours
        )
...
async def get_fee_comparison(
    ...
    try:
        ...
        for exchange in exchanges:
            ...
            estimated_fees = calculate_estimated_fees(fee_structure, trading_profile)
...
async def get_fee_breakdown(
    ...
    try:
        ...
        fee_breakdown = await portfolio_manager.get_detailed_fee_breakdown(
            user_id=current_user["username"], period=period
        )
        ...
        fee_categories = {
            "trading_fees": {
                ...
                "total": fee_breakdown.get("maker_fees", 0)
                + fee_breakdown.get("taker_fees", 0),
            },
            "funding_fees": {
                ...
                "total": fee_breakdown.get("deposit_fees", 0)
                + fee_breakdown.get("withdrawal_fees", 0)
                + fee_breakdown.get("transfer_fees", 0),
            },
            "financing_fees": {
                ...
                "total": fee_breakdown.get("margin_fees", 0)
                + fee_breakdown.get("borrowing_fees", 0)
                + fee_breakdown.get("funding_rate_fees", 0),
            },
            "other_fees": {
                ...
                "total": fee_breakdown.get("api_fees", 0)
                + fee_breakdown.get("inactivity_fees", 0)
                + fee_breakdown.get("conversion_fees", 0),
            },
        }
        ...
        for category, data in fee_categories.items():
            fee_percentages[category] = {
                "percentage": (
                    (data["total"] / total_fees * 100) if total_fees > 0 else 0
                ),
                "amount": data["total"],
            }
        ...
        cost_analysis = {
            ...
            "fees_as_percent_of_volume": (
                (total_fees / fee_breakdown.get("total_volume", 1)) * 100
                if fee_breakdown.get("total_volume", 0) > 0
                else 0
            ),
        }
...
def calculate_fee_metrics(fee_analysis: Dict[str, Any], period: str) -> Dict[str, Any]:
    ...
    return {
        "total_fees": total_fees,
        "average_fee_rate": (
            (total_fees / total_volume * 100) if total_volume > 0 else 0
        ),
        "fees_per_trade": total_fees / total_trades if total_trades > 0 else 0,
        "fees_as_percent_of_profit": (
            (total_fees / total_profit * 100) if total_profit > 0 else 0
        ),
...
def calculate_fee_efficiency(
    fee_analysis: Dict[str, Any], exchange_fees: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    efficiency = (
        (expected_rate - actual_rate) / expected_rate * 100 if expected_rate > 0 else 0
    )
...
def generate_fee_optimization_suggestions(
    ...
    for tier, requirements in exchange_fees.get("vip_tiers", {}).items():
        if monthly_volume >= requirements["volume_required"] * 0.8:
            potential_savings = (
                exchange_fees["taker_fee"] - requirements["taker"]
            ) * monthly_volume
            suggestions.append(
                {
                    "type": "vip_tier",
...
def get_fee_benchmarks(fee_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    benchmarks = {
        ...
        "active_trader": {
            "rate": 0.15,
            "description": "Active trader with some optimization",
        },
...
def calculate_optimization_scenarios(
    trade_size: float, frequency: int
) -> Dict[str, Any]:
    ...
    scenarios = {
        ...
        "optimized_maker": {
            ...
            "monthly_fees": monthly_volume
            * (0.8 * 0.0005 + 0.2 * 0.001),  # 80% maker, 20% taker
    ...
    for scenario in ["optimized_maker", "vip_tier"]:
        ...
        scenarios[scenario]["savings_percent"] = (
            (savings / current_fees * 100) if current_fees > 0 else 0
        )
...
def compare_exchange_fees(trade_size: float, frequency: int) -> Dict[str, Any]:
    ...
    for exchange in exchanges:
        ...
        effective_rate = (
            0.6 * fee_structure["maker_fee"] + 0.4 * fee_structure["taker_fee"]
        )
    ...
    return {
        ...
        "max_monthly_savings": max(
            [data["monthly_fees"] for data in comparison.values()]
        )
        - cheapest[1]["monthly_fees"],
    }
...
def analyze_vip_tier_benefits(trade_size: float, frequency: int) -> Dict[str, Any]:
    ...
    for tier, requirements in fee_structure.get("vip_tiers", {}).items():
        ...
        vip_analysis["tier_progress"][tier] = {
            ...
            "potential_savings": (fee_structure["taker_fee"] - requirements["taker"])
            * monthly_volume,
        }
            ...
            vip_analysis["recommendations"].append(
                {
                    ...
                    "potential_monthly_savings": (
                        fee_structure["taker_fee"] - requirements["taker"]
                    )
                    * monthly_volume,
                }
            )
...
def calculate_potential_savings(
    optimization_scenarios: Dict, order_type_analysis: Dict, vip_tier_analysis: Dict
) -> Dict[str, Any]:
    ...
    order_type_savings = (
        max([data["monthly_cost"] for data in order_type_analysis.values()])
        - current_cost
    )
    ...
    return {
        ...
        "annual_savings": {"total": total_monthly_savings * 12},
        "payback_period_months": 0,  # Immediate savings
        "roi_percent": float("inf") if total_monthly_savings > 0 else 0,
    }


def rank_exchanges_by_cost(
    exchange_comparison: Dict, trading_profile: Dict
) -> List[Dict[str, Any]]:
    ...
    for exchange, data in exchange_comparison.items():
        total_monthly_cost = (
            ...
            + data.get("withdrawal_fees", {}).get("btc", 0)
            * trading_profile.get("monthly_withdrawals", 1)
            * 50000  # Assume $50k BTC
...
def calculate_exchange_savings(
    exchange_comparison: Dict, current_exchange: str, trading_profile: Dict
) -> Dict[str, Any]:
    ...
    current_fees = exchange_comparison.get(current_exchange, {}).get(
        ...
        if exchange != current_exchange:
            ...
            if monthly_savings > 0:
                savings_opportunities[exchange] = {
                    ...
                    "savings_percent": (
                        (monthly_savings / current_fees * 100)
                        if current_fees > 0
                        else 0
                    ),
                }
...
def calculate_estimated_fees(
    fee_structure: Dict, trading_profile: Dict
) -> Dict[str, Any]:
    ...
    return {"monthly_fees": monthly_fees, "annual_fees": monthly_fees * 12}
