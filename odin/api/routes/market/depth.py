...
async def get_market_depth(
    ...
    try:
        ...
        best_bid = order_book["bids"][0]["price"] if order_book["bids"] else current_price * 0.999
        best_ask = order_book["asks"][0]["price"] if order_book["asks"] else current_price * 1.001
        
        spread_metrics = {
            "bid_ask_spread": best_ask - best_bid,
            "spread_bps": ((best_ask - best_bid) / current_price) * 10000,
            "mid_price": (best_bid + best_ask) / 2,
            "relative_spread": ((best_ask - best_bid) / ((best_bid + best_ask) / 2)) * 100
        }
...
async def get_market_impact(
    trade_sizes: List[float] = Query([1000, 5000, 10000, 25000, 50000], description="Trade sizes in USD"),
    rate_limiter = Depends(get_strategy_rate_limiter)
):
    ...
    try:
        ...
        for size_usd in trade_sizes:
            ...
            buy_impact = calculate_trade_impact(order_book["asks"], size_usd, "buy", current_price)
            sell_impact = calculate_trade_impact(order_book["bids"], size_usd, "sell", current_price)
            ...
            impact_analysis[f"{int(size_usd)}_usd"] = {
                ...
                "average_impact": (buy_impact["impact_bps"] + sell_impact["impact_bps"]) / 2
            }
...
async def get_liquidity_analysis(
    ...
    try:
        ...
        volume_metrics = {
            "current_volume": current_data.volume_24h if hasattr(current_data, 'volume_24h') else 0,
        ...
        liquidity_risk = {
            ...
            "recommendations": generate_liquidity_recommendations(liquidity_score, liquidity_metrics)
        }
...
async def get_order_flow_analysis(
    ...
    try:
        ...
        data_collector = BitcoinDataCollector()
        recent_data = await data_collector.get_recent_data(limit=hours * 2)  # Approximate
...
def calculate_depth_metrics(order_book: Dict[str, Any], current_price: float) -> Dict[str, Any]:
    ...
    for level in depth_levels:
        bid_depth = sum([
            order["value_usd"] for order in bids 
            if order["price"] >= current_price * (1 - level)
        ])
        
        ask_depth = sum([
            order["value_usd"] for order in asks 
            if order["price"] <= current_price * (1 + level)
        ])
...
def calculate_trade_impact(orders: List[Dict], trade_size_usd: float, side: str, current_price: float) -> Dict[str, Any]:
    ...
    if total_cost > 0:
        average_price = sum([o["price"] * o["filled_usd"] for o in filled_orders]) / sum([o["filled_usd"] for o in filled_orders])
    ...
    return {
        ...
        "fill_ratio": (trade_size_usd - remaining_size) / trade_size_usd if trade_size_usd > 0 else 0,
...
def calculate_liquidity_score(order_book: Dict[str, Any]) -> float:
    ...
    # Depth within 1%
    total_depth = sum([o["value_usd"] for o in bids[:10]]) + sum([o["value_usd"] for o in asks[:10]])
...
def calculate_comprehensive_liquidity_metrics(order_book: Dict, current_data, recent_data) -> Dict[str, Any]:
    ...
    return {
        ...
        "effective_spread_bps": ((asks[0]["price"] - bids[0]["price"]) / ((asks[0]["price"] + bids[0]["price"]) / 2)) * 10000,
        "depth_1_percent": sum([o["value_usd"] for o in bids[:5]]) + sum([o["value_usd"] for o in asks[:5]]),
...
def calculate_overall_liquidity_score(liquidity_metrics: Dict, volume_metrics: Dict, order_book: Dict) -> float:
    ...
    volume_score = min(100, volume_metrics.get("current_volume", 0) / 1000000)  # Normalize to 1M
    resilience_score = liquidity_metrics.get("resilience_score", 50)
    
    return (spread_score * 0.3 + depth_score * 0.3 + volume_score * 0.2 + resilience_score * 0.2)
...
def calculate_resilience_score(order_book: Dict) -> float:
    ...
    bid_variance = sum([(x - sum(bid_distribution)/len(bid_distribution))**2 for x in bid_distribution])
    ask_variance = sum([(x - sum(ask_distribution)/len(ask_distribution))**2 for x in ask_distribution])
...
def analyze_order_flow_patterns(recent_data: List, hours: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "flow_direction": "bullish" if price_momentum > 0 and volume_momentum > 0 else "bearish" if price_momentum < 0 and volume_momentum > 0 else "neutral",
...
def classify_order_flow(recent_data: List) -> Dict[str, Any]:
    ...
    return {
        ...
        "classification": "institutional" if institutional_score > 0.3 else "mixed" if institutional_score > 0.1 else "retail",
        "large_order_frequency": large_volume_count / len(volumes)
    }


def generate_flow_based_signals(flow_analysis: Dict, pressure_analysis: Dict) -> Dict[str, Any]:
    """Generate trading signals based on order flow"""
    signals = []
    
    if flow_analysis.get("flow_direction") == "bullish" and pressure_analysis.get("net_pressure", 0) > 0.2:
        signals.append({
            "signal": "BUY",
    ...
    elif flow_analysis.get("flow_direction") == "bearish" and pressure_analysis.get("net_pressure", 0) < -0.2:
        signals.append({
            "signal": "SELL", 
            "strength": min(1.0, flow_analysis.get("flow_strength", 0) * 2),
    ...
    return {
        "signals": signals,
        "overall_bias": "bullish" if pressure_analysis.get("net_pressure", 0) > 0 else "bearish" if pressure_analysis.get("net_pressure", 0) < 0 else "neutral",
