...
async def get_portfolio_performance(
    ...
    try:
        ...
        performance_kpis = {
            ...
            "current_drawdown_percent": performance_data.get(
        ...
        benchmark_data = await portfolio_manager.get_benchmark_performance(
            benchmark=benchmark, hours=validated_hours
        )

        # Calculate relative performance
        relative_performance = {
            "excess_return": performance_kpis["total_return_percent"]
            - benchmark_data.get("total_return_percent", 0),
...
async def get_returns_attribution(
    ...
    try:
        ...
        total_return = sum(
            attr.get("contribution_percent", 0)
            for attr in strategy_attribution.values()
        )
        ...
        for strategy, attr in strategy_attribution.items():
            attribution_summary[strategy] = {
                ...
                "contribution_ratio": (
                    attr.get("contribution_percent", 0) / total_return
                    if total_return != 0
                    else 0
                ),
        ...
        worst_performers = sorted(
            attribution_summary.items(), key=lambda x: x[1]["contribution_percent"]
        )[:3]
...
async def get_drawdown_analysis(
    ...
    try:
        ...
        drawdown_stats = {
            ...
            "current_drawdown_percent": drawdown_data.get(
                "current_drawdown_percent", 0
            ),
            "average_drawdown_percent": drawdown_data.get(
