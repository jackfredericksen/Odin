...
async def rebalance_portfolio(
    ...
    try:
        ...
        risk_assessment = await risk_manager.assess_rebalancing_risk(
            rebalancing_plan=rebalancing_plan, user_id=current_user["username"]
        )
        ...
        expected_impact = {
            ...
            "largest_trade_usd": max(
                [
                    trade.get("amount_usd", 0)
                    for trade in rebalancing_plan.get("trades", [])
                ],
                default=0,
            ),
        ...
        logger.info(
            f"Portfolio rebalancing {'executed' if execute else 'simulated'} for {current_user['username']}"
        )
...
async def get_rebalancing_recommendations(
    ...
    try:
        ...
        for category, recs in recommendation_categories.items():
            if recs:
                potential_impact[category] = {
                    ...
                    "max_allocation_change": max(
                        [abs(r.get("allocation_change", 0)) for r in recs]
                    ),
                    "estimated_improvement": sum(
                        [r.get("expected_improvement", 0) for r in recs]
                    ),
                }
        ...
        return {
            ...
            "last_rebalancing": await portfolio_manager.get_last_rebalancing_date(
    ...
    try:
        ...
        for strategy, target in target_allocations.items():
            ...
            allocation_deviations[strategy] = {
                ...
                "rebalancing_needed": abs(target - current)
                > allocation_update.get("rebalance_threshold", 0.05) * 100,
            }
...
async def get_rebalancing_history(
    ...
    try:
        ...
        if rebalancing_history:
            ...
            total_fees_paid = sum(
                [event.get("total_fees", 0) for event in rebalancing_history]
            )
            average_turnover = (
                sum([event.get("total_turnover", 0) for event in rebalancing_history])
                / total_rebalances
            )
            ...
            avg_performance_impact = (
                sum(performance_impact) / len(performance_impact)
