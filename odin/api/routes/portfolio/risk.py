...
async def get_exposure_analysis(
    ...
    try:
        ...
        # Concentration risk
        concentration_risk = {
            "largest_position_percent": max([exp["allocation_percent"] for exp in strategy_exposure.values()]),
            "top_3_concentration": sum(sorted([exp["allocation_percent"] for exp in strategy_exposure.values()], reverse=True)[:3]),
            "herfindahl_index": sum([exp["allocation_percent"]**2 for exp in strategy_exposure.values()]) / 10000,
            "effective_strategies": 1 / sum([exp["allocation_percent"]**2 for exp in strategy_exposure.values()]) * 10000 if any(exp["allocation_percent"] for exp in strategy_exposure.values()) else 0
        }
...
async def run_stress_test(
    ...
    try:
        valid_scenarios = [
            ...
            "flash_crash", "prolonged_bear", "black_swan"
        ]
        ...
        mitigation_suggestions = await risk_manager.generate_stress_mitigation_suggestions(
...
async def get_correlation_analysis(
    ...
    try:
        ...
        btc_correlations = {}
        for strategy in ["ma", "rsi", "bb", "macd"]:
            btc_correlations[strategy] = correlation_data.get(f"{strategy}_btc_correlation", 0)
        ...
        concentration_effect = {
            "correlation_adjusted_diversification": diversification_metrics["diversification_ratio"],
            "concentration_risk_level": correlation_risk,
            "recommended_action": "reduce_correlation" if correlation_risk == "high" else "maintain"
        }
    ...
    try:
        ...
        for strategy in ["ma", "rsi", "bb", "macd"]:
            ...
            strategy_risk_contributions[strategy] = {
                ...
                "risk_contribution_percent": (strategy_data.get("component_var", 0) / total_portfolio_risk * 100) if total_portfolio_risk > 0 else 0,
                "diversification_benefit": strategy_data.get("diversification_benefit", 0)
            }
