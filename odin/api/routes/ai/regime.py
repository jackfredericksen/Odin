...
from odin.ai.regime_detection.market_states import (
    MarketRegime,
    get_all_regime_info,
...
async def get_current_regime(db=Depends(get_database)) -> APIResponse:
    """
    ...
    try:
        ...
        if recent_data.empty:
            raise HTTPException(
                status_code=404, detail="No recent market data available"
            )
        ...
        regime_info = (
            get_regime_characteristics(MarketRegime(regime))
            if regime != "unknown"
            else None
        )
        ...
        response_data = {
            ...
            "regime_characteristics": {
                "description": (
                    regime_info.description if regime_info else "Unknown regime"
                ),
                "risk_level": regime_info.risk_level if regime_info else "Unknown",
                "trading_approach": (
                    regime_info.trading_approach if regime_info else "Cautious"
                ),
                "typical_indicators": (
                    regime_info.typical_indicators if regime_info else []
                ),
            },
...
async def get_regime_history(
    ...
    try:
                ...
                transitions.append(
                    {
                        ...
                        "duration_since_last": (
                            regime_history[i]["timestamp"]
                            - regime_history[i - 1]["timestamp"]
                        ).total_seconds()
                        / 3600,  # hours
                    }
                )
        ...
        for i in range(len(regime_history) - 1):
            ...
            duration = (
                regime_history[i + 1]["timestamp"] - regime_history[i]["timestamp"]
            )
        ...
        response_data = {
            ...
            "summary": {
                ...
                "most_common_regime": (
                    max(regime_counts.items(), key=lambda x: x[1].total_seconds())[0]
                    if regime_counts
                    else None
                ),
                "analysis_period": f"{days} days",
                "avg_regime_duration": (
                    round(total_duration.total_seconds() / len(regime_counts) / 3600, 1)
                    if regime_counts
                    else 0
                ),  # hours
            },
