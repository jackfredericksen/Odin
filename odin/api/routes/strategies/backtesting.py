...
async def backtest_strategy(
    ...
    initial_balance: float = Query(
        10000.0, description="Initial balance for backtesting"
    ),
    ...
    try:
        ...
        trading_metrics = {
            ...
            "net_profit_after_costs": backtest_result.get(
                "final_value", initial_balance
            )
            - initial_balance,
            "profit_factor": backtest_result.get("profit_factor", 1.0),
            "maximum_consecutive_losses": backtest_result.get(
...
async def custom_backtest(
    ...
    try:
        ...
        backtest_result = await strategy.custom_backtest(config=backtest_config)
