...
async def get_strategy_signals(
    ...
    signal_type: Optional[str] = Query(
        None, description="Filter by signal type (BUY/SELL)"
    ),
    ...
    try:
        signals = await strategy.get_historical_signals(
            hours=validated_hours, signal_type=signal_type
        )
        ...
        profitable_signals = [
            s for s in executed_signals if s.get("profit_loss", 0) > 0
        ]
        ...
        signal_stats = {
            ...
            "execution_rate": (
                len(executed_signals) / len(signals) * 100 if signals else 0
            ),
            "profitable_signals": len(profitable_signals),
            "win_rate": (
                len(profitable_signals) / len(executed_signals) * 100
                if executed_signals
                else 0
            ),
