            ...
            try:
                ...
                if live_perf and "error" not in live_perf:
                    ...
                    comparison_results[name] = {
                        ...
                        "backtest_return_percent": backtest_result.get(
                            "total_return_percent", 0
                        ),
                        "live_vs_backtest_diff": live_perf.get("return_percent", 0)
                        - backtest_result.get("total_return_percent", 0),
...
async def get_strategy_leaderboard(
    timeframe: str = Query(
        "7d", description="Timeframe for leaderboard (1d, 7d, 30d, 1y)"
    ),
    metric: str = Query(
        "return", description="Ranking metric (return, sharpe, win_rate)"
    ),
    rate_limiter=Depends(get_strategy_rate_limiter),
):
    ...
    try:
        ...
        timeframe_hours = {"1d": 24, "7d": 168, "30d": 720, "1y": 8760}
