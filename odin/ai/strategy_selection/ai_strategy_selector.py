...
class AIStrategySelector:
    ...
    def __init__(self, regime_detector: RegimeDetector, strategy_scorer: StrategyScorer):
        """
    ...
    async def select_optimal_strategy(self, price_data: List[PriceData], 
        ...
        try:
            ...
            if strategy_scores:
                ...
                # Fallback
                best_strategy_id = available_strategies[0] if available_strategies else None
                best_score = 0.5
            
            # Generate selection reasoning
            reasoning = self._generate_selection_reasoning(
                best_strategy_id, regime_result, detailed_analysis.get(best_strategy_id, {})
            )
            ...
            return {
                "strategy_id": available_strategies[0] if available_strategies else None,
                "confidence": 0.1,
    ...
    async def _calculate_strategy_score(self, strategy_id: str, strategy_type: str,
                                      regime_result: Dict[str, Any], 
                                      price_data: List[PriceData]) -> Dict[str, Any]:
        ...
        try:
            ...
            total_score = (
                ...
                performance_scores.get("recent_performance", 0.5) * self.weights["recent_performance"] +
                performance_scores.get("risk_adjusted_return", 0.5) * self.weights["risk_adjusted_return"] +
            ...
            return {
                ...
                "breakdown": {
                    ...
                    "recent_performance": performance_scores.get("recent_performance", 0.5),
                    "risk_adjusted_return": performance_scores.get("risk_adjusted_return", 0.5),
    ...
    def _get_regime_compatibility_score(self, strategy_type: str, 
                                      regime_result: Dict[str, Any]) -> float:
        ...
        try:
            ...
            base_score = self.strategy_regime_matrix[strategy_type].get(current_regime, 0.5)
            
            # Adjust by regime confidence
            confidence_adjusted_score = base_score * regime_confidence + 0.5 * (1 - regime_confidence)
    ...
    def _calculate_market_condition_score(self, strategy_type: str, 
                                        price_data: List[PriceData]) -> float:
        ...
        try:
            ...
            price_trend = (prices[-1] - prices[0]) / prices[0] if len(prices) >= 2 else 0
    ...
    def _generate_selection_reasoning(self, selected_strategy: str, 
                                    regime_result: Dict[str, Any],
        ...
        try:
            ...
            if strategy_type in self.strategy_regime_matrix:
                compatibility = self.strategy_regime_matrix[strategy_type].get(regime, 0.5)
                if compatibility > 0.7:
                    reasoning_parts.append(f"{strategy_type} strategy well-suited for {regime} conditions")
                elif compatibility > 0.5:
                    reasoning_parts.append(f"{strategy_type} strategy moderately suited for current conditions")
                else:
                    reasoning_parts.append(f"{strategy_type} strategy selected despite regime mismatch due to strong performance")
    ...
    async def get_strategy_recommendations(self, price_data: List[PriceData],
                                         available_strategies: List[str],
                                         top_n: int = 3) -> List[Dict[str, Any]]:
        """Get top N strategy recommendations with detailed analysis."""
        try:
            selection_result = await self.select_optimal_strategy(price_data, available_strategies)
            
            # Sort strategies by score
            sorted_strategies = sorted(
                selection_result["all_scores"].items(),
                key=lambda x: x[1],
                reverse=True
            )
    ...
    def get_configuration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_regimes": list(set().union(*[
                regime_dict.keys() 
