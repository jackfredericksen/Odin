...
class AdaptiveStrategyManager:
            ...
            RegimeType.BULL_TRENDING: RegimeStrategyMapping(
                ...
                primary_strategies=[
                    ...
                    StrategyConfig(
                        ...
                        parameters={"fast_period": 8, "slow_period": 21, "signal_period": 9},
                        weight=0.4,
                        min_confidence=0.6,
    ...
    def update_regime(self, regime: str, confidence: float) -> bool:
        ...
        try:
            ...
            if regime_changed:
                self.logger.info(f"Regime change detected: {self.current_regime} -> {new_regime}")
    ...
    def _adapt_strategies_for_regime(self, regime: RegimeType, confidence: float) -> bool:
        """
        ...
        try:
            ...
            self.logger.info(f"Activated {len(self.active_strategies)} strategies for {regime.value}")
            return True
        ...
        try:
            ...
            for strategy_info in self.active_strategies:
                ...
                try:
                    ...
                    self.logger.warning(f"Signal generation failed for {strategy.__class__.__name__}: {e}")
                    continue
    ...
    def _adjust_signal_for_regime(self, signal: Signal, config: StrategyConfig) -> Signal:
        ...
        try:
            ...
            adjusted_strength = max(0.1, min(1.0, adjusted_strength))  # Clamp between 0.1 and 1.0
    ...
    def _calculate_weighted_signal(self, buy_signals: List[Dict], sell_signals: List[Dict]) -> Dict:
    ...
    def update_strategy_performance(self, strategy_name: str, performance_metrics: Dict):
    ...
    def get_strategy_performance_by_regime(self, regime: RegimeType, days: int = 30) -> Dict:
        ...
        try:
            ...
            for strategy_name, performance_history in self.strategy_performance.items():
                regime_entries = [
                    ...
                    if entry['regime'] == regime.value and entry['timestamp'] > cutoff_date
                ]
                ...
                if regime_entries:
                    ...
                    for metric in metrics:
                        values = [entry['metrics'][metric] for entry in regime_entries if metric in entry['metrics']]
                        if values:
    ...
    def optimize_strategy_weights(self, lookback_days: int = 30):
        ...
        try:
            ...
            for strategy_name, perf_data in regime_performance.items():
                ...
                strategy_scores[strategy_name] = max(0.1, score)  # Minimum weight of 0.1
            
            # Update weights in current regime mapping
            if strategy_scores:
                self._update_regime_strategy_weights(strategy_scores)
                self.logger.info(f"Updated strategy weights for {self.current_regime.value}")
    ...
    def get_regime_info(self) -> Dict:
        ...
        return {
            ...
            'active_strategy_count': len(self.active_strategies),
            'regime_risk_multiplier': self.regime_strategy_map[self.current_regime].risk_multiplier if self.current_regime else 1.0,
            'max_exposure': self.regime_strategy_map[self.current_regime].max_exposure if self.current_regime else 0.5
        }
    ...
    def _log_regime_change(self, new_regime: RegimeType, confidence: float):
        ...
        try:
            regime_change_entry = {
                'timestamp': datetime.now(),
                'from_regime': self.current_regime.value if self.current_regime else 'unknown',
            ...
            if len(self.regime_performance['regime_changes']) > 1000:
                self.regime_performance['regime_changes'] = self.regime_performance['regime_changes'][-1000:]
    ...
    def _save_performance_data(self):
        ...
        try:
            performance_file = os.path.join(self.config_path, 'strategy_performance.json')
            ...
            if os.path.exists(performance_file):
                ...
                for strategy, history in data.items():
                    ...
                    for entry in history:
                        ...
                        converted_entry['timestamp'] = datetime.fromisoformat(entry['timestamp'])
