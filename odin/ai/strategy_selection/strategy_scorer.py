...
class StrategyScorer:
    ...
    async def calculate_strategy_scores(self, strategy_id: str, 
                                      price_data: List[PriceData]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            # 4. Drawdown Score
            scores["drawdown_score"] = self._calculate_drawdown_score(
                performance_data
            )
            
            # 5. Win Rate Score
            scores["win_rate_score"] = self._calculate_win_rate_score(
                performance_data
            )
    ...
    async def _get_strategy_performance_data(self, strategy_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Get recent signals
            signals = self.database.get_recent_signals(strategy_id=strategy_id, limit=50)
    ...
    def _calculate_recent_performance_score(self, performance_data: Dict[str, Any]) -> float:
    ...
    def _calculate_adaptability_score(self, performance_data: Dict[str, Any], 
    ...
    def _calculate_overall_score(self, scores: Dict[str, float]) -> float:
        ...
        try:
            ...
            overall = (
                ...
                scores.get("consistency_score", 0.5) * self.metric_weights["consistency"] +
                scores.get("risk_adjusted_return", 0.5) * self.metric_weights["risk_adjusted"] +
            ...
            overall = overall * (0.8 + 0.4 * adaptability)  # Scale between 0.8x and 1.2x
    ...
    def _cache_results(self, strategy_id: str, scores: Dict[str, float]):
        ...
        self.cache_expiry[strategy_id] = datetime.now(timezone.utc) + self.cache_duration
    ...
    async def compare_strategies(self, strategy_ids: List[str], 
        ...
        try:
            ...
            return {
                ...
                "best_strategy": strategy_comparisons[0] if strategy_comparisons else None,
                "worst_strategy": strategy_comparisons[-1] if strategy_comparisons else None,
                "average_score": mean([s["overall_score"] for s in strategy_comparisons]) if strategy_comparisons else 0
            }
    ...
    async def get_performance_insights(self, strategy_id: str) -> List[str]:
        ...
        try:
            ...
            if win_rate > 0.6:
                ...
                insights.append(f"Low win rate of {win_rate:.1%} - consider optimization")
            ...
            trade_returns = performance_data.get("trade_returns", [])
            if len(trade_returns) > 5:
                cv = stdev(trade_returns) / abs(mean(trade_returns)) if mean(trade_returns) != 0 else float('inf')
                if cv < 1.0:
            ...
            return insights if insights else ["Strategy performance within normal ranges"]
    ...
    async def analyze_strategy_trends(self, strategy_id: str, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            if not daily_performance:
                return {"trend": "no_recent_data", "analysis": "No recent trades in specified period"}
    ...
    def _generate_trend_analysis(self, trend: str, avg_return: float, volatility: float) -> str:
