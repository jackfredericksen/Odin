/**
 * Odin Bitcoin Trading Dashboard - Main JavaScript
 */
...
class Dashboard {
    ...
    updateBitcoinPrice(data) {
        ...
        if (timestampElement) {
            timestampElement.textContent = `Last updated: ${this.formatTime(data.timestamp)}`;
        }
    }
        ...
        card.innerHTML = `
            ...
            <div class="strategy-metrics">
                <div class="strategy-metric">
                    ...
                    <span class="value ${strategy.return >= 0 ? 'text-success' : 'text-danger'}">
        ...
        orders.forEach(order => {
            ...
            row.innerHTML = `
                ...
                <td class="${order.pnl >= 0 ? 'text-success' : 'text-danger'}">
                    ${order.pnl >= 0 ? '+' : ''}${this.formatCurrency(order.pnl)}
                </td>
            `;
    ...
    async toggleAutoTrading() {
        try {
            const endpoint = this.isAutoTradingEnabled ? '/trading/disable' : '/trading/enable';
    ...
    async showStrategyDetails(strategy) {
        ...
        if (body) {
            body.innerHTML = `
                ...
                <div class="strategy-actions" style="margin-top: 1rem; display: flex; gap: 0.5rem;">
