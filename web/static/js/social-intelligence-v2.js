/**
 * Social Intelligence Module v2
 * Handles Twitter/X, Reddit, News feeds with manual configuration
 */
...
class SocialIntelligence {
    ...
    async init() {
        ...
        console.log('✅ Social Intelligence initialized');
    }
    ...
    renderUI() {
        ...
        content.innerHTML = `
            ...
            <div id="social-settings-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 1000; align-items: center; justify-content: center;">
                <div style="background: var(--bg-panel); border: 1px solid var(--border-primary); border-radius: 8px; padding: 2rem; max-width: 600px; width: 90%; max-height: 80vh; overflow-y: auto;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                        <h2 style="margin: 0; font-size: 1.25rem;">⚙️ Social Feed Settings</h2>
                    ...
                    <div style="margin-bottom: 1.5rem;">
                        <label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">📰 News Sources</label>
                    ...
                    <!-- Subreddits -->
                    <div style="margin-bottom: 1.5rem;">
                        <label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">📱 Subreddits</label>
                    ...
                    <!-- Twitter Keywords -->
                    <div style="margin-bottom: 1.5rem;">
                        <label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">🐦 Twitter Keywords</label>
            ...
            <div class="terminal-panel" style="margin-bottom: var(--spacing-lg);">
                <div class="terminal-panel-header">
                    ...
                    <div style="display: flex; gap: 0.5rem;">
                        <button class="panel-toggle" onclick="window.socialIntelligence.openSettings()" title="Settings">⚙️</button>
                ...
                <div class="terminal-panel-body">
                    <div style="display: grid; grid-template-columns: 1fr 2fr; gap: var(--spacing-lg);">
                        ...
                        <div style="text-align: center;">
                            ...
                            <div id="sentiment-gauge" style="font-size: 4rem; margin: 1rem 0;">🟡</div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: var(--spacing-md);">
                ...
                <div class="terminal-panel">
                    <div class="terminal-panel-header">
                        <span class="terminal-panel-title">📰 News</span>
                    </div>
    ...
    async loadTwitterFeed() {
        try {
            ...
            const response = await fetch(`${this.apiBase}/social/twitter?keywords=${keywords}`);
            const result = await response.json();
    ...
    async loadRedditFeed() {
        try {
            ...
            const response = await fetch(`${this.apiBase}/social/reddit?subreddits=${subreddits}`);
            const result = await response.json();
            ...
            <div style="padding: 1rem; border-bottom: 1px solid var(--border-subtle);">
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                    <div style="flex: 1;">
                        ...
                        <div style="font-size: 0.75rem; color: var(--text-secondary); margin-top: 0.25rem;">r/${post.subreddit} • u/${post.author}</div>
                    </div>
    ...
    async loadNewsFeed() {
        try {
            ...
            const response = await fetch(`${this.apiBase}/social/news?sources=${sources}`);
            const result = await response.json();
            ...
            <div style="padding: 1rem; border-bottom: 1px solid var(--border-subtle);">
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                    <div style="flex: 1;">
                        ...
                        <div style="font-size: 0.75rem; color: var(--text-secondary); margin-top: 0.25rem;">${article.source.toUpperCase()} • ${this.formatTimeAgo(article.published_at)}</div>
                    </div>
    ...
    startAutoUpdate() {
        ...
        console.log(`🔄 Social Intelligence auto-update started (${this.updateInterval / 1000}s interval)`);
    }
    ...
    stopAutoUpdate() {
        if (this.intervalId) {
            clearInterval(this.intervalId);
            this.intervalId = null;
            console.log('⏹️ Social Intelligence auto-update stopped');
        }
    }
    ...
    openSettings() {
        ...
        document.getElementById('source-coindesk').checked = this.settings.newsSources.includes('coindesk');
        document.getElementById('source-decrypt').checked = this.settings.newsSources.includes('decrypt');
        document.getElementById('source-cointelegraph').checked = this.settings.newsSources.includes('cointelegraph');

        document.getElementById('subreddits-input').value = this.settings.subreddits.join(', ');
        document.getElementById('twitter-keywords-input').value = this.settings.twitterKeywords.join(', ');
        ...
        document.getElementById('refresh-interval-container').style.display = this.settings.autoRefresh ? 'block' : 'none';
    ...
    /**
     * Update news sources from checkboxes
     */
    updateNewsSources() {
        const sources = [];
        if (document.getElementById('source-coindesk').checked) sources.push('coindesk');
        if (document.getElementById('source-decrypt').checked) sources.push('decrypt');
        if (document.getElementById('source-cointelegraph').checked) sources.push('cointelegraph');
    ...
    updateAutoRefresh() {
        ...
        document.getElementById('refresh-interval-container').style.display = enabled ? 'block' : 'none';
    }
    ...
    resetSettings() {
        ...
        this.saveSettings();
        this.openSettings(); // Refresh the modal with defaults
    }

    /**
     * Apply settings and reload feeds
     */
    async applySettings() {
        ...
        const twitterInput = document.getElementById('twitter-keywords-input').value;
        this.settings.twitterKeywords = twitterInput.split(',').map(s => s.trim()).filter(s => s);
        ...
        console.log('✅ Settings applied and feeds reloaded');
    }
}
